		...
		private bool CheckLines( int x, int y, int x_e, int y_e, int count ){
			...
			if (c == 0){
				while (true)	{
					...
					if ((size >= 5) && (count == 3)){
						if ((board[x + i * x_e, y + i * y_e] == Empty) && (board[x + (i+1) * x_e, y + (i+1) * y_e] == Cross)){
							if (CanPlace(x + (i+4) * x_e, y + (i+4) * y_e))	{
								board[x + (i+4) * x_e, y + (i+4) * y_e] = Nought;
							return true;
							}
						}
							else if ((board[x + i * x_e, y + i * y_e] == Cross) && 
								(board[x + (i+1) * x_e, y + (i+1) * y_e] == Cross) &&
								(board[x + (i+4) * x_e, y + (i+4) * y_e] == Empty)){
									if (CanPlace(x + (i+3) * x_e, y + (i+3) * y_e)){
										board[x + (i+3) * x_e, y + (i+3) * y_e] = Nought;
										return true;
									}
								}
							else if ((board[x + i * x_e, y + i * y_e] == Empty) && 
									(board[x + (i+1) * x_e, y + (i+1) * y_e] == Empty)){
										if (CanPlace(x + (i+1) * x_e, y + (i+1) * y_e))	{
											board[x + (i+1) * x_e, y + (i+1) * y_e] = Nought;
											return true;
										}
								}
						}
		...
		private bool CheckDiagonals ( int i, int count ){
			...
			if (c == 0){
				while (true){
					...
					if ((size>=5) && (count == 3)){
						if ((board[i,q]==Empty)&& (board[i+1,q+1]==Cross)){
							if (CanPlace(i + 4, q + 4)){
							board[i + 4, q + 4] = Nought;
							return true;
							}
						}
						else if ((board[i,q]==Cross)&& (board[i+1,q+1] == Cross) && (board[i+4,q+4]  ==Empty)){
								if (CanPlace(i + 3, q + 3)){
									board[i + 3, q + 3] = Nought;
								return true;
								}
							}
							else if ((board[i,q]==Empty)&& (board[i+1,q+1]==Empty)){
									if (CanPlace(i + 1, q + 1)){
										board[i + 1, q + 1] = Nought;
									return true;
									}
								}
					}
