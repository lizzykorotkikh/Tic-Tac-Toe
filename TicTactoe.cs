...
public class TicTacToe {
	...
	private void ComputerStep(){
		...
		for ( int i = 0; i < combinations.Length; ++i )//checking every single combinations, where computer need to pay attention
			if ( LineDivision( combinations[i].count, combinations[i].symbol) ){// calling the submethod
	...
	private bool LineDivision (int count, Square symb){//divide the board into the lines (rows, columns, diagonals)
...
class View : Form{
	...
	void Game ( Graphics g ){
		...
		if ( t.winner > 0 && t.winner < 3){
			g.FillRectangle( new SolidBrush(Color.Yellow), t.wy1, t.wx1, 1, 1); 
			g.FillRectangle( new SolidBrush(Color.Yellow), t.wy2, t.wx2, 1, 1); 
			g.FillRectangle( new SolidBrush(Color.Yellow), 2*t.wy2 - t.wy1, 2*t.wx2 - t.wx1, 1, 1); 
		}
		...
		if (s == 9){
			...
			if ( level == 1 ){
				...
				g.DrawString (level1, new Font("Comic Sans MS", 9), new SolidBrush(Color.DarkBlue), new RectangleF(10, 10, 380, 40) );
			}
			else{
				string level2 = @"This level is with two players. The first player will be X.
					                   The second player will be O.";
				g.DrawString (level2, new Font("Comic Sans MS", 9), new SolidBrush(Color.DarkBlue), new RectangleF(40, 10, 350, 40) );
			}
